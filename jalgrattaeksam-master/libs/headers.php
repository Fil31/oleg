...
if ($page_check == 'registreerimine.php') {
    if (isset($_REQUEST["sisestusnupp"])) {
        if (!empty($_REQUEST["eesnimi"])) {
            ...
            $kask = $yhendus->prepare("INSERT INTO jalgrattaeksam(eesnimi) VALUES (?)");
            $kask->bind_param("s", $_REQUEST["eesnimi"]);
            $kask->execute();
            $yhendus->close();
...
if ($page_check == 'slaalom.php') {
    if (!empty($_REQUEST["teooriatulemus"])) {
        ...
        if (!preg_match("#[A-z]#", $tulemus)) {
            $kask = $yhendus->prepare("UPDATE jalgrattaeksam SET hind2=? WHERE id=?");
            $kask->bind_param("ii", $tulemus, $_REQUEST["id"]);
            $kask->execute();
